...
TestCube::TestCube(Graphics& gfx, float size)
{
	...
	AddBind(PixelShader::Resolve(gfx, "Shader\\PhongPS.cso"));
	...
	auto tcbdb = std::make_shared<TransformCBufVtxPxl>(gfx, *this, 0u, 2u);
	AddBind(tcbdb);
	...
	pvs = VertexShader::Resolve(gfx, "Shader\\SolidVS.cso");
	pvsbc = pvs->GetBytecode();
	outlineEffect.push_back(std::move(pvs));
	outlineEffect.push_back(PixelShader::Resolve(gfx, "Shader\\SolidPS.cso"));
