	...
	modelPath( path.string() )
{
	...
	// phong technique
	{
		...
		// common (pre)
		vtxLayout.Append( TemplateVertex::VertexLayout::Position3D );
		vtxLayout.Append( TemplateVertex::VertexLayout::Normal );
		...
		// diffuse
		{
			...
			if( material.GetTexture( aiTextureType_DIFFUSE,0,&texFileName ) == aiReturn_SUCCESS )
			{
				hasTexture = true;
				shaderCode += "Dif";
				vtxLayout.Append( TemplateVertex::VertexLayout::Texture2D );
		...
		// normal
		{
			if( material.GetTexture( aiTextureType_NORMALS,0,&texFileName ) == aiReturn_SUCCESS )
			{
				...
				shaderCode += "Nrm";
				vtxLayout.Append( TemplateVertex::VertexLayout::Texture2D );
				vtxLayout.Append( TemplateVertex::VertexLayout::Tangent );
				vtxLayout.Append( TemplateVertex::VertexLayout::Bitangent );
	...
	// outline technique
	{
		...
		}
		{
			...
			draw.AddBindable( PixelShader::Resolve( gfx,"Shader\\Solid_PS.cso" ) );
...
TemplateVertex::VertexBuffer Material::ExtractVertices( const aiMesh& mesh ) const noexcept
{
...
std::shared_ptr<Bind::VertexBuffer> Material::MakeVertexBindable(Graphics& gfx, const aiMesh& mesh, float scale) const noxnd
{
	...
	if (scale != 1.0f)
	{
		...
		{
			DirectX::XMFLOAT3& pos = vtc[i].Attr<TemplateVertex::VertexLayout::ElementType::Position3D>();
			pos.x *= scale;
			pos.y *= scale;
			pos.z *= scale;
		}
